...
$title = '<h1 align="center" style="font-size: 15px;">Liste des produit en rupture stock (Boutique)</h1>';
if ($printable == 1) {
    $title = '<h1 align="center" style="font-size: 15px;">Liste des produit en stock (Boutique)</h1>';
}elseif ($printable == 2) {
...
foreach ($produits as $produit) {
    ...
    if ($printable == 1) {
        $table .= '<td>' . $stock_function->stock_boutique_by_produit($produit->proid) . '</td>';
    }elseif ($printable == 2) {
        $table .= '<td><span class="text-warning">' . $stock_function->stock_boutique_by_produit($produit->proid) . '</span> <span style="font-font-style:italic;; font-size: 10px;">(en alert)</span></td>';
    }else {
